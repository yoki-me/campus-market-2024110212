...
onMounted(async () => {
  try {
    ...
    items.value = res.data.map((e: ErrandItem) => ({ ...e, type: 'errands' } as CampusItem))
...
async function toggle(id: string) { await favoritesStore.toggleFavorite(id, 'errands') }
</script>

<template>
  <div class="page anim-up">
    <div class="breadcrumb"><RouterLink to="/">首页</RouterLink> / <span>跑腿委托</span></div>
    <div class="page-header">
      <h1 class="page-title">跑腿委托</h1>
      <p class="page-sub">代取代办，校园互助</p>
    </div>

    <div v-if="loading" class="loading-box"><div class="spinner"></div></div>
    <div v-else-if="!items.length" class="empty-box">
      <EmptyState text="暂无跑腿委托信息" />
    ...
    <div v-else class="items-grid">
      <div v-for="item in items" :key="item.id" style="position:relative">
        <ItemCard :item="item" @click="router.push(`/detail/errands/${item.id}`)" />
