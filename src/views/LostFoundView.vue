...
onMounted(async () => {
  try {
    ...
    items.value = res.data.map((lf: LostFoundItem) => ({
      ...lf,
      type: 'lostFounds' as const,
      lostOrFound: lf.type, // db.json 中用 type 字段表示 lost/found
    } as CampusItem))
...
<template>
  <div class="page anim-up">
    <div class="breadcrumb"><RouterLink to="/">首页</RouterLink> / <span>失物招领</span></div>
    <div class="page-header">
      <h1 class="page-title">失物招领</h1>
      <p class="page-sub">遗失与拾获，互帮互助</p>
    </div>

    <div v-if="loading" class="loading-box"><div class="spinner"></div></div>
    <div v-else-if="!items.length" class="empty-box">
      <EmptyState text="暂无失物招领信息" />
